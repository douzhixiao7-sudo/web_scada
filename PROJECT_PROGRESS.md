...
更新时间：2026-09-20 11:37（Asia/Shanghai）

## 当前项目定位

本项目当前处于 MVP 骨架阶段，已经把 Web SCADA 的后台基础能力和金斗河现场点表主线跑通：工程结构、数据库连接、登录菜单、设备台账、点位台账、系统字典、采集通道配置、Redis 实时当前值、实时监控页面、报警闭环和报警规则维护 MVP。当前还没有进入真实 PLC 采集、控制下发、历史数据或组态画面阶段。
...
- 未做真实实时采集任务；已完成采集通道配置和点位绑定模型。
- 未做实时值数据库表；当前值已由内置仿真采集器写入 Redis。
- 未做 WebSocket / SSE 推送。
- 未做控制下发。
...
- 已完成通道与点位绑定模型。
- 已完成内置仿真采集调度服务，后续可替换为独立 Modbus TCP 仿真 PLC 或真实采集器。
...
下一阶段建议进入“独立 Modbus TCP 仿真 PLC”：让仿真数据源从后端内部逻辑独立出来，Java 后端通过 Modbus 客户端采集它，再写入 Redis 当前值。这样链路会更接近真实现场。
