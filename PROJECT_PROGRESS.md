...
更新时间：2026-09-20 09:21（Asia/Shanghai）

## 当前项目定位

本项目当前处于 MVP 骨架阶段，已经把 Web SCADA 的后台基础能力和金斗河现场点表主线跑通：工程结构、数据库连接、登录菜单、设备台账、点位台账、系统字典和实时监控模拟数据页面。当前还没有进入真实 PLC 采集、控制下发、报警规则维护、历史数据或组态画面阶段。
...
- 未做 WebSocket / SSE 推送。
- 未做控制下发。
- 已做报警闭环 MVP；未做报警规则维护页面、历史高级筛选和报表。
- 未做历史数据、趋势曲线和报表。
- 未做 HMI 组态画面。
...
- 已支持报警确认、恢复状态、处理备注和历史事件查询；后续需补规则维护和高级筛选。
