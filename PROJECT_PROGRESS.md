...
更新时间：2026-09-20 00:07（Asia/Shanghai）

## 当前项目定位

本项目当前处于 MVP 骨架阶段，已经把 Web SCADA 的后台基础能力和金斗河现场点表主线跑通：工程结构、数据库连接、登录菜单、设备台账、点位台账、系统字典和实时监控模拟数据页面。当前还没有进入真实 PLC 采集、控制下发、报警闭环、历史数据或组态画面阶段。
...
### 11. 报警规则 MVP 与 Modbus 数据区

- 已新增 `/api/alarms/active` 活动报警接口。
- 当前报警按实时模拟值即时计算，不落报警事件表。
- 规则包含质量异常、数据超时、故障信号触发、开度越限、电流偏高、电压越限。
- 前端“报警中心”已从静态演示改为接口驱动。
...
- 未做 WebSocket / SSE 推送。
- 未做控制下发。
- 已做报警规则 MVP；未做报警确认、恢复、历史事件和规则维护页面。
- 未做历史数据、趋势曲线和报表。
- 未做 HMI 组态画面。
...
- 支持报警确认、恢复、处理备注和历史事件查询。
- 当前报警中心已接接口，后续需要增加筛选、确认和历史追溯。
